...
export const getCollidingIndexes = (layout: Layout, item: LayoutItem): string[] => {
	return layout.filter((elem) => elem.id !== item.id && isOverlap(item, elem)).map((item) => item.id);
};
 ...
 * @returns
 */
export const getItemSizeStyle = (width: number, height: number, x: number, y: number, gap: number, column: number, row: number) => {
