<template>
	<div ref="gridItemRef" draggable="true" class="grid-item-box" @dragstart="dragStartHandle" @dragend="dragEndHandle">
		<div
			:class="{ 'grid-item': true, 'static-item': itemLayout.static || props.draggableFrom, 'grid-resizing': isDraging && handleType === 'resize' }"
		>
			<slot />
			<div v-show="resize && !itemLayout.static" @pointerdown.stop.prevent="(e: PointerEvent) => draggableStart(e, 'resize')">
				<slot name="resize">
					<div class="grid-item-resize"></div>
				</slot>
			</div>
			<!-- 这里绑定pointerdown事件主要是为了防止鼠标按下事件的扩散，从而触发到父元素的拖拽事件 -->
			<div v-show="remove && !itemLayout.static" @pointerdown.stop.prevent="draggableStart" @click.stop.prevent="removes(props.id)">
				<slot name="remove">
...
onMounted(() => {
	...
	if (itemLayout.value.static || !drag.value) return;
...
const dragStartHandle = (e: DragEvent) => {
	...
	isDraging.value = true;
	dragStore.set(props.group, props.data);
};
